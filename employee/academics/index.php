 ...
 *
 * This page is used to manage the features for Employee->Academics. 
							...
							<div class="card">
								<div class="card-body">
									<ul class="nav nav-tabs" id="custom-content-below-tab" role="tablist">
										<li class="nav-item">
											<a class="nav-link" id="custom-content-below-myschedule-tab" data-toggle="pill" href="#custom-content-below-myschedule" role="tab" aria-controls="custom-content-below-myschedule" aria-selected="true">My Class Schedule</a>
										</li>
										<li class="nav-item">
											<a class="nav-link " id="custom-content-below-myadvisory-tab" data-toggle="pill" href="#custom-content-below-myadvisory" role="tab" aria-controls="custom-content-below-myadvisory" aria-selected="true">My Advisory</a>
										</li>	
									</ul>
									<div class="tab-content" id="custom-content-below-tabContent">
										<div class="tab-pane fade show" id="custom-content-below-myschedule" role="tabpanel" aria-labelledby="custom-content-below-myschedule-tab">
											<br>	
											<div class="row">
												<div class="col-md-8">
												...
												<div class="col-md-4">
													<select class="form-control" onchange="changeTerm();" id="academics-myterms">

													</select>
												</div>
											</div>
											<div class="table-responsive" id="academics-myschedule">
											</div>									
										</div>
										<div class="tab-pane fade show" id="custom-content-below-myadvisory" role="tabpanel" aria-labelledby="custom-content-below-myadvisory-tab">
											<?php 
											...
											<div class="row">
												...
												<div class="col-md-2">
													<button type="button" class="btn btn-info float-right" id="enrol-learner" name="enrol-learner" onclick="enrolLearner();">Enrol Learner</button>
												</div>
											</div>
											<br>
...
function getScheduleTerms(){
	...
	$.ajax({
		type: 'POST',
		url: 'academics/action.php',
		data: {data:data},	
		success: function(result){
			$('#academics-myterms').html(result);
		}
	});	
}
...
function getSchedules(){
	...
	$.ajax({
		...
		data: {data:data},	
		success: function(result){
			$('#academics-myschedule').html(result);
		}
	});	
}

function changeTerm(){
	currentSY = $('#academics-myterms').val();
	getSchedules();
}

function getClassInfo(cid){
	var action = 'getClassInfo';
	var data = [action, cid];
	...
	$.ajax({
		...
		data: {data:data},	
		success: function(result){
			$('#level').html(result.section_level);
			getClassList(cid, result.section_level);
			$('.modal-title').html('Submit Grades for <strong>'+result.pros_title+' (Gr '+result.section_level+', '+result.section_name+')</strong>');

		}
	});		
}

function getClassList(cid, level){
	var action = 'getClassList';
	var data = [action, cid, '%', level];
	...
	$.ajax({
		...
		data: {data:data},	
		success: function(result){
			$('#grade-submit').html(result);
		}
	});		
}

function submitGrades(){
	var action = 'submitGrades';
	var success = true;
	
	var grade_no = new Array();
	$('input[name^="grade_no"]').each(function() { grade_no.push($(this).val()); });
	
	var grade_q1 = new Array();
	$('input[name^="grade_q1"]').each(function() { sanitizeForm($(this).val()) == true ? grade_q1.push($(this).val()) : success = false; });
	
	var grade_q2 = new Array();
	$('input[name^="grade_q2"]').each(function() { sanitizeForm($(this).val()) == true ? grade_q2.push($(this).val()) : success = false; });
	
	var grade_q3 = new Array();
	$('input[name^="grade_q3"]').each(function() { sanitizeForm($(this).val()) == true ? grade_q3.push($(this).val()) : success = false; });
	
	var grade_q4 = new Array();
	$('input[name^="grade_q4"]').each(function() { sanitizeForm($(this).val()) == true ? grade_q4.push($(this).val()) : success = false; });
	
	var data = [action, grade_no, grade_q1, grade_q2, grade_q3, grade_q4, $('#level').html(), id];		
		...
		$.ajax({
			...
			success: function(result){
				if(result[0] == 1){
					...
					getClassInfo($('#class_no').html());
				} else {
					toastr.error(result[1]+'');
				}
				
				setTimeout(function(){document.getElementById('submit').innerHTML = result[1];}, 1000);
				setTimeout(function(){document.getElementById('submit').innerHTML = 'Submit Grades';}, 2000);
				setTimeout(function(){$('#submit').removeAttr('disabled');}, 3000);
			}
		});	
	} else {
		toastr.error('Invalid grade entry.');
	}
}

function sanitizeForm(grade){
	var success = true;
	
	if(grade == 0){
		success = true;
	}else if(grade > <?php echo $max_grade;?>){
		success = false;
	} else if (grade < <?php echo $min_grade;?>) {
		success = false;
	}
...
function checkAdvisorhsip(){
	var action = 'checkAdvisorhsip';
	var data = [action, id, currentSY];
	...
	$.ajax({
		...
		success: function(result){
			if(result[0] == 1){
				$('#custom-content-below-myadvisory-tab').toggleClass('active');
				$('#custom-content-below-myadvisory').toggleClass('active');
				getAdvisory();
			} else {
				$('#custom-content-below-myschedule-tab').toggleClass('active');
				$('#custom-content-below-myschedule').toggleClass('active');
				$('#custom-content-below-myadvisory-tab').hide();
				$('#custom-content-below-myadvisory').hide();
			}
		}
	});
}
...
function enrolLearner(){
	window.location = '?p=admissions&section='+sid;
}
</script>